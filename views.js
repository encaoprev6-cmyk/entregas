import { Deliveries, Vehicles, Drivers, Neighborhoods, CostCategories, ReturnReasons, Cycles, OdometerLogs, Costs, AuditLog, Counters } from './db.js';
import { $, $$, money, dateBR, dateTimeBR, escapeHtml, toast, badge, STATUS_META, guardClick, downloadCSV, downloadJSON, wirePhoneMask } from './helpers.js';
import { getEnv, closeModal, openModal, refreshApp } from './app.js';
...
export async function renderCentral() {
  ...
  const atrasadas = rows.filter((r) => {
    ...
    return mins > 60; // limite configurável — ver Configurações
  });

  const vehicles = await Vehicles.all();
  const kmPendente = (await OdometerLogs.all()).filter((k) => k.kmEnd == null).length;
  const custosPendentes = 0; // custos não têm "pendência" própria neste modelo — reservado
  ...
  return `
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px">
      <div class="mascot-frame">
        ...
        <div class="mascot-mood" id="mascotMood">🙂</div>
      </div>
      <div>
        <div style="font-weight:800;font-size:16px">${greet}! Aqui está o resumo de agora.</div>
        <div style="color:var(--text-muted);font-size:12.5px">${env === 'treino' ? 'Você está no modo Treinamento — nada aqui entra no histórico oficial.' : 'Operação Real'}</div>
      </div>
    </div>

    <div class="stat-row">
      <div class="stat-card"><small>Na loja</small><strong>${naLoja.length}</strong></div>
      <div class="stat-card"><small>Em rota</small><strong>${emRota.length}</strong></div>
      <div class="stat-card"><small>Prioritárias</small><strong>${prioritarias.length}</strong></div>
      <div class="stat-card ${atrasadas.length ? 'accent' : ''}"><small>Atrasadas (&gt;60min)</small><strong>${atrasadas.length}</strong></div>
    </div>
    <div class="stat-row">
      <div class="stat-card"><small>Ciclos ativos</small><strong>${openCycles.length}</strong></div>
      <div class="stat-card"><small>Para reentrega</small><strong>${reentrega.length}</strong></div>
      <div class="stat-card"><small>Agendadas</small><strong>${agendadas.length}</strong></div>
      <div class="stat-card"><small>KM pendente</small><strong>${kmPendente}</strong></div>
    </div>
...
export async function renderDashboard() {
  ...
  const problems = rows.filter((r) => ['retorno', 'cancelada'].includes(r.status));
  const financial = await computeFinancialSummary(env);

  const drivers = await Drivers.all();
  const byDriver = {};
  finalized.forEach((d) => { byDriver[d.driverId] = (byDriver[d.driverId] || 0) + 1; });
  const ranking = Object.entries(byDriver).sort((a, b) => b[1] - a[1]).slice(0, 5)
    .map(([id, count]) => `<tr><td>${escapeHtml(drivers.find((d) => d.id === id)?.name || 'Sem entregador')}</td><td>${count}</td></tr>`).join('');

  return `
    <div class="stat-row">
      <div class="stat-card"><small>Total de entregas</small><strong>${rows.length}</strong></div>
      <div class="stat-card"><small>Finalizadas</small><strong>${finalized.length}</strong></div>
      <div class="stat-card"><small>Retorno/Canceladas</small><strong>${problems.length}</strong></div>
      <div class="stat-card accent"><small>Saldo financeiro</small><strong>${money(financial.balance)}</strong></div>
    </div>
    ${rows.length === 0 ? `<div class="empty-state"><strong>Sem dados ainda</strong>Os números aparecem assim que você registrar entregas.</div>` :
    `<h3 style="font-size:14px;margin:20px 0 10px">Ranking por entregador (finalizadas)</h3>
    <div class="table-wrap"><table><thead><tr><th>Entregador</th><th>Entregas</th></tr></thead><tbody>${ranking || '<tr><td colspan="2" style="color:var(--text-muted)">Sem dados suficientes</td></tr>'}</tbody></table></div>`}
  `;
}
  ...
  return `
    <div class="stat-row" style="grid-template-columns:1fr 1fr">
      <div class="stat-card">
        ...
        <p style="font-size:12.5px;color:var(--text-muted);margin:8px 0">Resumo com totais, pronto para impressão/PDF.</p>
        <button class="btn-primary btn-small" id="printReportBtn">🖶 Imprimir / Salvar PDF</button>
      </div>
      <div class="stat-card">
        <small>Relatório analítico</small>
        <p style="font-size:12.5px;color:var(--text-muted);margin:8px 0">Todos os campos, histórico e financeiro em CSV (abre no Excel).</p>
...
export function wireReportsEvents() {
  $('#printReportBtn')?.addEventListener('click', async () => {
    ...
    area.innerHTML = `
      ...
      <table border="1" cellpadding="6" style="border-collapse:collapse;width:100%">
        ...
        <tr><td>Finalizadas</td><td>${rows.filter(r=>r.status==='finalizada').length}</td></tr>
  ...
  $('#exportCsvBtn')?.addEventListener('click', async () => {
    ...
    const neighborhoods = await Neighborhoods.all();
    const nName = (id) => neighborhoods.find((n) => n.id === id)?.name || '';
    const header = ['Compra','Chegada','Data entrada','PDV','DOC','Cupom','Cliente','Telefone','Endereço','Bairro','Taxa','Tamanho','Prioridade','Tipo','Status','Reembolsado','Observações'];
    const lines = rows.map((r) => [r.purchaseNumber, r.arrivalNumber, dateTimeBR(r.entryTime), r.pdv, r.doc, r.coupon, r.clientName, r.phone, r.street, nName(r.neighborhoodId), r.deliveryFee, r.size, r.priority, r.type, STATUS_META[r.status]?.label, r.refunded ? 'Sim' : 'Não', r.notes]);
    downloadCSV(`orbita-relatorio-${env}-${new Date().toISOString().slice(0,10)}.csv`, [header, ...lines]);
...
export async function renderRegistry(tab = 'vehicles') {
  const tabs = [
    ['vehicles', 'Veículos'], ['drivers', 'Entregadores'], ['neighborhoods', 'Bairros'],
  ...
  let listHtml = '';
  if (tab === 'vehicles') listHtml = await registryList(Vehicles, 'label', 'vehicles');
...
const REGISTRY_STORES = { vehicles: Vehicles, drivers: Drivers, neighborhoods: Neighborhoods, costCategories: CostCategories, returnReasons: ReturnReasons };
const REGISTRY_FIELD = { vehicles: 'label', drivers: 'name', neighborhoods: 'name', costCategories: 'name', returnReasons: 'label' };
