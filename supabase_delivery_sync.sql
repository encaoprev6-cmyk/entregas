-- Nilo Entregas • instalação inicial do banco
-- Execute antes de supabase_roles_and_audit.sql no SQL Editor do Supabase.
...
create table if not exists public.delivery_sync_entities (
  ...
  constraint delivery_sync_entities_type check (entity_type in (
    'meta','settings','vehicles','neighborhoods','employees','costCategories',
    'reasons','deliveries','cycles','routeTracks','odometerLogs','costs',
    'audit','dayClosures','trash'
  )),
...
begin
  ...
  if tg_op = 'UPDATE' then
    if new.client_updated_at < old.client_updated_at then return old; end if;
...
on public.delivery_workspaces for select to authenticated
using (active and exists (
  select 1 from public.delivery_workspace_members member
...
on public.delivery_workspace_members for select to authenticated
...
on public.delivery_sync_entities for select to authenticated
using (exists (
  select 1 from public.delivery_workspace_members member
...
on public.delivery_sync_entities for insert to authenticated
with check (exists (
  select 1 from public.delivery_workspace_members member
  where member.workspace_id = delivery_sync_entities.workspace_id
    and member.user_id = (select auth.uid()) and member.active
...
on public.delivery_sync_entities for update to authenticated
using (exists (
  select 1 from public.delivery_workspace_members member
  where member.workspace_id = delivery_sync_entities.workspace_id
    and member.user_id = (select auth.uid()) and member.active
...
grant select on public.delivery_workspaces, public.delivery_workspace_members to authenticated;
